...
void main()
{
	...
	try
	{
		...
		if ((connect(cC, (sockaddr*)&serv, sizeof(serv))) == SOCKET_ERROR) // подключение к серверу
		...
		for (int i = 0; i < count; i++)
		{
			...
			if (SOCKET_ERROR == send(cC, obuf, sizeof(obuf), NULL)) //отправка
				cout << "send:" << GetLastError() << endl;;

			if (SOCKET_ERROR == recv(cC, ibuf, sizeof(ibuf), NULL)) // получение
		...
		if (WSACleanup() == SOCKET_ERROR) //освобождение библиотеки
