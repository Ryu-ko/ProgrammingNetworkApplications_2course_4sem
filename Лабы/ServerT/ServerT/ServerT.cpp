...
void main()
{
	...
	try
	{
		...
		if (bind(sS, (LPSOCKADDR)&serv, sizeof(serv)) == SOCKET_ERROR) // привязка порта, хоста, сем проток к сокету
			throw  SetErrorMsgText("bind:", WSAGetLastError());

		if (listen(sS, SOMAXCONN) == SOCKET_ERROR) // слушаем пока придут с клиента сообщения
		...
		sockaddr_in c_adr; // сюда придет адрес сокета  
		sockaddr_in s_adr;
		{
		...
		//--------------------------------------------------
		if (closesocket(sS) == SOCKET_ERROR) //очистка сокета
			throw  SetErrorMsgText("closesocket:", WSAGetLastError());
		if (WSACleanup() == SOCKET_ERROR)
			throw  SetErrorMsgText("Cleanup:", WSAGetLastError()); //очиска библиотек
	}
