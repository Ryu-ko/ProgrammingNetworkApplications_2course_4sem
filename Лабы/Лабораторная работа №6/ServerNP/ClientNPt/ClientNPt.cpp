...
int main()
{
    ...
    try {
        ...
        if (!SetNamedPipeHandleState(cH, &mode, NULL, NULL)) { //----------- макс колво байт интерв задержки
            cout << GetLastError();
        ...
        for (int i = 1; i <= countMessage; i++) {
            ...
            if (!TransactNamedPipe(cH, obuf, sizeof(obuf), ibuf, sizeof(ibuf), &lp, NULL)) { //-------обедин чтения и записи---------
